...
static void getMAFBlock2(Block *block, FILE *fileHandle) {
    if (block_getLength(block) >= minimumBlockLength) {
        ...
        if (referenceNumber > 0 || sampleNumber > 0) {
            stIntTuple *key = stIntTuple_construct(2, referenceNumber,
                    sampleNumber);
            int32_t *value;
    ...
    while ((event = eventTree_getNext(eventIt)) != NULL) {
        ...
        if (eventString != NULL && strcmp(eventString, referenceEventString)
            ...
            for (int32_t i = 0; i < stList_length(copyNumbers); i++) {
                ...
                totalBaseCount += stIntTuple_getPosition(copyNumber, 1)
                        * columnCount[0];
            }
            fprintf(
                    fileHandle,
                ...
                if (assemblyNumber < referenceNumber) {
                    ...
                    if (assemblyNumber > 0) {
                        totalCopyNumberDeficientColumnsGreaterThanZero
                                += columnCount[0];
                        totalCopyNumberDeficientBasesGreaterThanZero
                                += columnCount[0] * (referenceNumber
                                        - assemblyNumber);
                    }
                } else if (assemblyNumber > referenceNumber) {
                    totalCopyNumberExcessColumns += columnCount[0];
                    totalCopyNumberExcessBases += columnCount[0]
                            * (assemblyNumber - referenceNumber);
                }
            }
            fprintf(
                    fileHandle,
                    "<deficientCopyNumberCounts totalColumns=\"%i\" totalBases=\"%i\" totalProportionOfColumns=\"%f\" totalProportionOfBases=\"%f\"/>",
                    totalCopyNumberDeficientColumns,
                    totalCopyNumberDeficientBases,
                    ((float) totalCopyNumberDeficientColumns)
                            / totalColumnCount,
            ...
            fprintf(
                    ...
                    totalCopyNumberDeficientColumnsGreaterThanZero,
                    totalCopyNumberDeficientBasesGreaterThanZero,
                    ((float) totalCopyNumberDeficientColumnsGreaterThanZero)
                            / totalColumnCount,
                    ((float) totalCopyNumberDeficientBasesGreaterThanZero)
                            / totalBaseCount);
            fprintf(
                    fileHandle,
