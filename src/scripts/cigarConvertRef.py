...
def fastaFilesToRefs(fastaFiles):
    refs = {} #key = 'shortName' (e.g: hg19.chr6), val = Ref object
    for fastaFile in fastaFiles:
        for header, sequence in fastaRead(open(fastaFile, 'r')):
            ref = Ref(header.split()[0])
            if ref.name in refs:
...
def main():
    usage = ('%prog infile outfile fasta-files')
    parser = OptionParser( usage = usage )
    options, args = parser.parse_args()
    if len(args) < 3:
        raise ValueError("Required: inputfile, reffile, and fasta-files\n")
    infile = args[0]
    outfile = args[1]
    refs = fastaFilesToRefs(args[2:])
